...
static in_addr_t forward_addr = 0;
...
#endif
		{
			if(proto==e->ip.proto)
			{
				...
				*iport = ntohs(mr->range[0].min.all);
				...
				fiaddr.s_addr = ntohl(forward_addr) ;
				ciaddr.s_addr = ntohl(iaddr) ;
				syslog(LOG_INFO, "Get addr:%s:%u: ipaddr:%s:%u:\n", inet_ntoa(fiaddr ), fiaddr.s_addr, inet_ntoa(ciaddr), ciaddr.s_addr) ;
				if(fiaddr.s_addr == ciaddr.s_addr) {
					syslog(LOG_INFO, "Match Found %s :%s:%u: :%s:%u:\n", desc, inet_ntoa(fiaddr ), fiaddr.s_addr, inet_ntoa(ciaddr), ciaddr.s_addr) ;
					r = 0;
					break;
				}
			}
...
int
set_forward_addr(const char *iaddr)
{
	if(iaddr) {
		forward_addr = inet_addr(iaddr);
	}
	return forward_addr;
}
	...
	else
	{
...
#endif
		{
			if(proto==e->ip.proto)
			{
				...
				fiaddr.s_addr = ntohl(forward_addr) ;
				ciaddr.s_addr = ntohl(iaddr) ;
				syslog(LOG_INFO, "Forward addr:%s:%u: ipaddr:%s:%u:\n", inet_ntoa(fiaddr ), fiaddr.s_addr, inet_ntoa(ciaddr), ciaddr.s_addr) ;
				if(fiaddr.s_addr == ciaddr.s_addr) {
					syslog(LOG_INFO, "Match Found %i :%s:%u: :%s:%u:\n", i, inet_ntoa(fiaddr ), fiaddr.s_addr, inet_ntoa(ciaddr), ciaddr.s_addr) ;
					r = 0;
					break;
				}
			}
