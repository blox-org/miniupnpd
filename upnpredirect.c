...
upnp_redirect_internal(const char * rhost, unsigned short eport,
                       ...
                       unsigned int timestamp)
{
	syslog(LOG_INFO, "redirecting port %s:%hu to %s:%hu protocol %d for: %s",
		rhost, eport, iaddr, iport, proto, desc);
	/* Transcoding need LAN Interface IP, which require portmapping (portfowarding)
     and firewall pinholing. Check LAN Interface, if IP is 127.0.0.1, then dont forward just do pinholing */
