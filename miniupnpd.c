...
init(int argc, char * * argv, struct runtime_vars * v)
{
	...
	int transcoding_forward_addr = 0;
	...
	else
	{
		for(i=0; i<(int)num_options; i++)
		{
			...
			case UPNPLISTENING_IP:
				...
				if(!transcoding_forward_addr) /* Set forward add if transcoding forward addr is not set */
				{
					set_forward_addr(ary_options[i].value);
				}
			...
			case UPNPENABLENATPMP:
				...
				break;
#ifdef ENABLE_NATPMP_AUTH
			case UPNPNATPMPSECRET:
				ConfNATPMPAuth(ary_options[i].value);
				break;
#endif
#endif
#ifdef ENABLE_TRANSCODING
			case UPNPTRANSDSPADDR:
				ConfTransDspAddress(ary_options[i].value);
				set_forward_addr(ary_options[i].value);
				transcoding_forward_addr = 1;
				break;
#endif
...
main(int argc, char * * argv)
{
	...
	{
		if(OpenAndConfNATPMPSockets(snatpmp,v.port) < 0)
		{
